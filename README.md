...
위 명령어를 통해서 필요한 패키지를 설치합니다.

.env 파일을 생성하여 OPENAI_API_KEY, TAVILY_API_KEY를 적어주세요.

python -m uvicorn server:app --reload 로 서버를 실행합니다.

http://127.0.0.1:8000로 서비스를 확인합니다.
...
1. search_career_documents
   : 사용자가 업로드한 채용 관련 pdf 문서를 검색하는 Tool로 Chroma Vector DB 기반 Retriever를 호출하며, 사용자의 질의와 의미적으로 관련된 문서 Chunk를 검색한다.

2. search_official_recruitment_web
   : 기업 또는 공공기관의 채용 관련 정보를 검색하는 Tool로 Tavily Search를 사용하며 공식 도메인을 기준으로 검색한다.

3. general_web_search
   : 공식 채용자료와 PDF 문서만으로 정보가 부족한 경우 사용하는 보조 웹 검색 Tool로 Tavily Search를 사용하여 부족한 정보를 보완한다.
...
InMemorySaver(checkpointer) : LangGraph 상태를 저장하고 thread_id별로 대화 상태를 구분한다.
